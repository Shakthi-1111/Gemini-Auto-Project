...
# 1. Securely fetch the API key from GitHub Secrets
api_key = os.environ.get("GEMINI_API_KEY")
genai.configure(api_key=api_key)

# 2. Set up the current AI model
...
# 4. Ask Gemini to improve it
prompt = f"Refactor and improve this code. Return ONLY the raw code, no markdown formatting or explanations:\n\n{current_code}"
response = model.generate_content(prompt)

# 5. Save the new code and generate the Dashboard
if response.text:
    # Clean up the output and overwrite my_script.py
